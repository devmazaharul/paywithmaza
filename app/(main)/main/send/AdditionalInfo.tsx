    ...
    <div className="p-3 border border-gray-800 rounded-2xl shadow-2xl ">
      ...
      <div className="space-y-2 text-sm">
        <div className="flex items-center justify-between text-gray-300">
          ...
          <b className="text-emerald-500 text-base">{formatBDT(userIno?.item.balance as number)}</b>
        </div>
        <div className="flex items-center justify-between text-gray-300">
          <span>This Month Sent</span>
          <span>{formatBDT(totalReceivedAmount)}</span>
        </div>
        <div className="flex items-center justify-between text-red-600">
          <span>This Month Sent</span>
          <span>{formatBDT(totalSendAmount)}</span>
        </div>
          ...
          {transInfo && transInfo.map((recipient, index) => (
            ...
            >
              ...
              <div className="text-right">
                <p className={ `${recipient.type=="debit"?"text-red-500":" text-white"} `}>{formatBDT(recipient.amount)}</p>
